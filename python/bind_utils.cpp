...
to_ndarray_copyless(Sequence* seq_ptr)
{
        // Create a Python object that will free the allocated memory when destroyed:
	auto capsule = 
        py::capsule(seq_ptr, [](void* p){ delete reinterpret_cast<Sequence*>(p); });
...
struct ArrayWrapper
{
     ...
     * @return [description]
     */
    inline py::array_t<T> as_array()
    {
        return to_ndarray_copyless(vector_ptr_);
    }

    inline py::array_t<T> as_array(size_t n_cols)
    {
        size_t n_rows(size() / n_cols);

        auto out_array = as_array();
...
/**
* \brief Converts and STL Sequence to numpy array by copying it
