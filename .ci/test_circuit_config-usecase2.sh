...
spatial-index-circuit segments "${circuit_config_file}" -o "${segments_spi}"
python3 << EOF
...
spatial-index-circuit segments "${circuit_config_file}" --populations "NodeA" -o "${segments_NodeA_spi}"
python3 << EOF
import spatial_index
index = spatial_index.open_index("${segments_spi}")
sphere = [0.0, 0.0, 0.0], 1000.0
