...
brain-indexer-synapses --progress-bar "${edges_file}" -o "${direct_spi}"
brain-indexer-circuit synapses --progress-bar "${circuit_config_file}" -o "${circuit_spi}"
...
then
    ...
    srun -n${n_mpi_ranks} brain-indexer-synapses "${edges_file}" -o "${multi_direct_spi}" --multi-index
    srun -n${n_mpi_ranks} brain-indexer-circuit synapses "${circuit_config_file}" -o "${multi_circuit_spi}" --multi-index
